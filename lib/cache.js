...
const domain = '952793-2.myshopify.com'
const storefrontAccessToken = '500d1b19b23f5ea456f1aa0dc1f08b8d'
const countries = ['DE', 'TR', 'GB'];
...
const collection = `
{
  ...
  const tr = `query allProducts @inContext(country: TR) {
	products(first: 100) {
	  edges {
		node {
		  id
...
const fetchInternational = async () => {
    ...
    let internationalProducts = await Promise.all(countries.map(async (country) => {
        ...
        const data = await options.request(internationalQuery);
        return JSON.parse(JSON.stringify(data));
    }));
...
const fetchCommon = async () => {
	const [products, collections,international] = await Promise.all([fetchMerchant(), fetchCollection(),fetchInternational()])
	return {  collections, products,international }
}
